...
const root = `/tmp/horsemd-ordered-exit-before-list-${process.pid}`
...
// The failing region from 无序列表测试.md: an ordered list followed by a bullet
// list. Pressing Enter at the end of the last ordered item creates an empty
// item, and Enter inside that empty item exits the list — leaving Crepe's
// standalone `<br />` empty-paragraph placeholder between the two lists. The
// list-slot fingerprint must treat that placeholder like the authored blank
// line it stands for instead of a hard group fence.
const fixture = `# 测试
...
try {
  ...
  // Enter at the end of the last ordered item creates item 3, then Enter inside
  // that empty item removes it and exits the list.
  await focusParagraphEnd(app, '的人多不多吧')
  await rawKey(app.send, 'Enter', 'Enter', 13, '')
  await rawKey(app.send, 'Enter', 'Enter', 13, '')
  await sleep(1100)

  const before = await diagnostics(app)
  console.log('ORDERED_EXIT_AFTER_INPUT:', JSON.stringify({
  ...
  const source = await waitFor(() => visibleSource(app), 'source textarea did not open').catch(async (error) => {
    console.error('ORDERED_EXIT_DIAGNOSTICS:', JSON.stringify(await diagnostics(app), null, 2))
    throw error
  })
  const after = await diagnostics(app)
  console.log('ORDERED_EXIT_SOURCE:', JSON.stringify({ source, toasts: after.toasts }))
  assert.equal(
    after.toasts.some((text) => /保存已暂停|无法安全映射|原文件未被覆盖|Save paused/.test(text)),
    false,
    'exiting the empty ordered item showed a sync warning'
  )
  assert.match(source, /1\. 输入个人的时光\n2\. 的人多不多吧\n/, 'the ordered list body must survive the exit')
  assert.doesNotMatch(source, /(?:^|\n)3\. /, 'the emptied third item must be removed, not left as a dangling row')
  assert.match(source, /- 看了呢分/, 'the following bullet list must keep its authored dash marker')
  assert.doesNotMatch(source, /<br/, 'the standalone empty-paragraph placeholder must not leak into source')
  console.log('PASS ordered exit before a following list: empty item removal keeps list groups intact')
} finally {
