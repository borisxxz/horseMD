...
// E0 P3c acceptance — the 0.13.170 16:38:54 trace: Chinese IME composition in
// a top-level paragraph, commit, then an IMMEDIATE Enter at the paragraph end.
// The plain-paragraph-terminal-split family must own the whole journal: the
// author's paragraph bytes/BOM/CRLF survive, the new empty paragraph stays an
// editor-owned transient (never serialized), and source/save/cold-reopen stay
// warning-free.
const root = `/tmp/horsemd-plain-split-trailing-${process.pid}`
const basePort = Number(process.env.CDP_PORT || 13250 + (process.pid % 30))
const warningPattern = /检测到富文本与源码不一致|源码.*不一致|保存已暂停|无法安全映射|原文件未被覆盖|Save paused/i

const fixture = '\uFEFF' + [
  '# plain paragraph terminal split',
  '',
  '- authored bullet',
  '',
  'before',
...
const committed = 'target请问'
// Minimal separator: the authored blank boundary after the paragraph already
// exists, so the terminal split adds NO extra bytes; the empty paragraph
// itself is never serialized.
const expected = fixture.replace('target', committed)
...
const openApp = async ({ file, profile, port, expectedParagraphs }) => {
    ...
    await waitFor(
      () => app.evaluate(`(() => {
        ...
        const paragraphs = [...(editor?.querySelectorAll('p') || [])]
          .filter((node) => node.closest('blockquote') == null && node.closest('li') == null && (node.textContent || '') !== '')
          .map((node) => node.textContent || '')
        return Boolean(
          (editor.textContent || '').includes('authored bullet') &&
          JSON.stringify(paragraphs) === ${JSON.stringify(JSON.stringify(expectedParagraphs))}
        )
      })()`),
...
const clearDiagnostics = (app) => app.evaluate(`(() => {
  ...
  window.__hmPlainParagraphTransactionTrace = []
...
const focusEndOfParagraph = async (app, value) => {
  const result = await app.evaluate(`(() => {
    ...
    const paragraph = [...(editor?.querySelectorAll('p') || [])]
      .find((node) =>
        ...
        node.closest('blockquote') == null && node.closest('li') == null && (node.textContent || '') !== '')
  ...
  assert.equal(result.ok, true, `could not focus paragraph: ${JSON.stringify(result)}`)
  await sleep(80)
}

const imeComposeAndCommit = async (app, pinyin, cjk) => {
  const replacementId = `plain-split-${Date.now()}`
...
const snapshot = (app) => app.evaluate(`(() => {
  const editor = ${visibleEditor()}
  return {
    paragraphTexts: [...(editor?.querySelectorAll('p') || [])]
      .filter((node) => node.closest('blockquote') == null && node.closest('li') == null && (node.textContent || '') !== '')
      .map((node) => node.textContent || ''),
...
const assertSourceView = (source, label) => {
  ...
  assert.equal(source.includes(`${committed}\n\n- following bullet`), true,
    `${label} lost the authored split boundary`)
...
const assertSplitPublication = (state, expectedBoundary, label) => {
  ...
  const split = state.preserve.find((entry) =>
    entry.reason === 'plain-paragraph-split' &&
    entry.preserved === true &&
    entry.integrityProof?.kind === 'transaction-plain-paragraph-split-proof'
  )
  assert.ok(split, `${label} did not publish the terminal split owner: ${JSON.stringify(state.preserve)}`)
  assert.equal(split.integrityProof.family, 'plain-paragraph-terminal-split')
  assert.equal(split.integrityProof.terminalSplit, true)
  ...
  assert.equal(state.coordinator.some((entry) =>
    ...
    entry.family === 'plain-paragraph-terminal-split' &&
    entry.reason === 'plain-paragraph-split' &&
...
const reopenAndVerify = async ({ file, profile, port, label }) => {
  const app = await openApp({
    ...
    profile,
    port,
    expectedParagraphs: ['before', committed]
  })
  try {
...
const runScenario = async ({ name, immediateFlush, port }) => {
  ...
  const app = await openApp({
    ...
    profile: `${name}-edit`,
    port,
    expectedParagraphs: ['before', 'target']
  })
  try {
    await clearDiagnostics(app)
    await focusEndOfParagraph(app, 'target')
    ...
    assertSplitPublication(
      state,
      immediateFlush
        ? 'transaction-plain-paragraph-split-forced-flush'
        : 'transaction-plain-paragraph-split-markdown-updated',
      name
    )
...
try {
  ...
  completed = true
  console.log('PASS transaction-owned plain paragraph terminal split: IME composition + immediate Enter at the paragraph end preserves authored bytes/BOM/CRLF through source view, save and cold reopen without warnings')
} finally {
