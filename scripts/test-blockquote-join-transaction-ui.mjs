...
const root = `/tmp/horsemd-blockquote-join-${process.pid}`
const basePort = Number(process.env.CDP_PORT || 13280 + (process.pid % 30))
...
const joinedText = 'quotedXYalpha'

const fixture = '\uFEFF' + [
  '# blockquote join transaction',
  '',
  '- authored bullet',
  '',
  '  >   quoted',
...
const openApp = async ({ file, profile, port, expectedQuoteTexts }) => {
  ...
  try {
    await waitFor(
      ...
      `blockquote join editor did not mount for ${profile}`
    )
    await sleep(450)
    return app
  } catch (error) {
...
const focusRightParagraphStart = async (app) => {
  const result = await app.evaluate(`(() => {
    const editor = ${visibleEditor()}
    const paragraph = [...(editor?.querySelectorAll('blockquote p') || [])]
      .find((node) => (node.textContent || '') === ${JSON.stringify(rightText)})
    if (!editor || !paragraph) return { ok: false, reason: 'right-quote-paragraph-not-found' }
    const walker = document.createTreeWalker(paragraph, NodeFilter.SHOW_TEXT)
    const target = walker.nextNode()
    if (!target) return { ok: false, reason: 'right-quote-text-node-not-found' }
  ...
  assert.equal(result.ok, true, `could not focus quote join point: ${JSON.stringify(result)}`)
...
const assertSourceView = (source, label) => {
  ...
  assert.equal(source, expectedView, `${label} source differs from join owner output`)
  ...
  assert.equal(source.includes('  >   quotedXYalpha\n'), true,
    `${label} lost authored left quote prefix or merged text`)
  assert.equal(source.includes('  >\n'), false, `${label} retained quote-only separator`)
  assert.equal(source.includes('  >   alpha\n'), false, `${label} retained right quote prefix`)
...
const assertPublication = (state, expectedBoundary, label) => {
  assert.deepEqual(state.quoteTexts, [joinedText],
    `${label} rich quote join differs: ${JSON.stringify(state.quoteTexts)}`)
  ...
  assert.equal(captures.length >= 2, true,
    `${label} did not capture Backspace + follow-up transactions: ${JSON.stringify(state.journalTrace)}`)
  assert.equal(new Set(captures.map((entry) => entry.journalId)).size, 1,
    `${label} restarted journal during join batch: ${JSON.stringify(captures)}`)
  ...
  const preservation = state.preserve.find((entry) =>
    entry.reason === 'blockquote-paragraph-join' &&
    entry.preserved === true &&
    entry.integrityProof?.kind === 'transaction-blockquote-join-proof'
  )
  assert.ok(preservation, `${label} did not publish join owner: ${JSON.stringify(state.preserve)}`)
  assert.equal(preservation.integrityProof.family, 'blockquote-paragraph-join')
  assert.deepEqual(preservation.integrityProof.nodePath, [1, 0, 1])
  assert.equal(preservation.integrityProof.rightIndex, 1)
  assert.equal(preservation.integrityProof.leftText, leftText)
  assert.equal(preservation.integrityProof.rightText, rightText)
  assert.equal(preservation.integrityProof.mergedText, joinedText)
  assert.equal(preservation.integrityProof.joinStepName, 'ReplaceStep')
  assert.equal(preservation.integrityProof.joinTo - preservation.integrityProof.joinFrom, 2)
  ...
  assert.equal(state.integrity.some((entry) =>
    entry.preservationReason === 'blockquote-paragraph-join' &&
    entry.semanticOk === true &&
    entry.listSlotsMatch === true &&
    entry.ok === true
  ), true, `${label} join candidate was not fully equivalent: ${JSON.stringify(state.integrity)}`)

  assert.equal(state.coordinator.some((entry) =>
    entry.phase === 'published' &&
    entry.owner === 'transaction' &&
    entry.family === 'blockquote-paragraph-join' &&
    entry.boundary === expectedBoundary
  ), true, `${label} bypassed Coordinator join boundary: ${JSON.stringify(state.coordinator)}`)
  ...
  assert.ok(ownerPublication,
    `${label} did not emit join owner trace: ${JSON.stringify(state.ownerTrace)}`)
...
const reopenAndVerify = async ({ file, profile, port, label }) => {
  const app = await openApp({
    ...
    profile,
    port,
    expectedQuoteTexts: [joinedText]
  })
  try {
    const state = await snapshot(app)
    assert.deepEqual(state.quoteTexts, [joinedText], `${label} cold reopen lost join`)
...
const runScenario = async ({ name, immediateFlush, port }) => {
  ...
  try {
    ...
    await focusRightParagraphStart(app)
    ...
    const state = await snapshot(app)
    console.log(`${name.toUpperCase()}_AFTER_BLOCKQUOTE_JOIN:`, JSON.stringify(state))
    assertPublication(
      state,
      immediateFlush
        ? 'transaction-blockquote-join-forced-flush'
        : 'transaction-blockquote-join-markdown-updated',
      name
    )
    ...
    assert.equal(await readFile(file, 'utf8'), expected,
      `${name} saved bytes differ from join owner output`)
  } finally {
    await stopBuiltElectron(app, { removeProfile: true })
    app = null
  }
...
try {
  ...
  completed = true
  console.log('PASS transaction-owned blockquote join UI: physical Backspace + rapid text preserve stable nested path, authored quote prefix, BOM/CRLF, neighbours, callback/forced flush, source, save and cold reopen')
} finally {
