...
const root = `/tmp/horsemd-list-nested-last-child-outdent-retirement-${process.pid}`
const file = join(root, 'wide-marker-spacing.md')
const port = Number(process.env.CDP_PORT || 22920 + (process.pid % 30))
...
try {
  ...
  const point = await app.evaluate(`(() => {
    ...
    const item = [...(editor?.querySelectorAll('ul ul li') || [])]
      .find((candidate) => (candidate.querySelector('p')?.textContent || '').trim() === 'delta')
  ...
  await waitFor(() => app.evaluate(`(() =>
    (window.__hmListNestedLastChildBulletOutdentTransactionTrace || []).some((entry) =>
      entry.phase === 'plan' && entry.family === 'list-nested-last-child-bullet-outdent' &&
      entry.reason === 'nested-last-child-outdent-source-row-unproven' &&
      entry.recognized === true && entry.legacyBlocked === true
    )
  )()`), 'wide marker-spacing last-child outdent did not fail closed')
  await sleep(250)
  ...
  const blocked = state.owner.filter((entry) =>
    entry.phase === 'plan' && entry.reason === 'nested-last-child-outdent-source-row-unproven'
  )
  ...
  assert.equal(state.preserve.some((entry) => entry.reason === 'list-nested-last-child-bullet-outdented'), false)
  ...
  assert.deepEqual(state.topTexts, ['alpha', 'beta', 'delta', 'omega'])
  assert.deepEqual(state.nestedTexts, ['gamma'])
  assert.equal(await readFile(file, 'utf8'), fixture, 'fail-closed last-child outdent overwrote disk')

  completed = true
  console.log('PASS nested last-child bullet outdent legacy retirement UI: two-space marker padding on the target is recognized by the exact last-child lift family, blocks single/broad/legacy fallback, retains rich outdent, warns, and leaves disk untouched')
} finally {
