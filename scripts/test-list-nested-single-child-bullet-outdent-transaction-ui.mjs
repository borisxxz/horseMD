...
const root = `/tmp/horsemd-list-nested-single-child-outdent-${process.pid}`
const basePort = Number(process.env.CDP_PORT || 22220 + (process.pid % 30))
const packagedAppPath = process.env.HORSEMD_APP_PATH || ''
const scenarios = [
  {
    name: 'callback-parent-second',
    forced: false,
    marker: '+',
    target: 'gamma',
    fixture: '\uFEFFbefore\r\n\r\n+ alpha\r\n+ beta\r\n  + gamma\r\n\r\nafter\r\n',
...
const shape = (app) => app.evaluate(`(() => {
  ...
  return {
    ...
    topTexts: [...(top?.querySelectorAll(':scope > .milkdown-list-item-block > li') || [])]
      .map((item) => (item.querySelector(':scope > .children > .content-dom > p')?.textContent || '').trim())
  }
})()`)
...
const openApp = async ({ file, profile, port, indented }) => {
  ...
  try {
    await waitFor(async () => {
      ...
      return current.topCount === (indented ? 2 : 3) && current.nestedCount === (indented ? 1 : 0)
...
const focusNestedTarget = async (app, target) => {
  const point = await waitFor(() => app.evaluate(`(() => {
    const editor = ${visibleEditor()}
    const nestedLists = [...(editor?.querySelectorAll('ul ul') || [])]
    const item = nestedLists.flatMap((list) => [...list.querySelectorAll(':scope > .milkdown-list-item-block > li')])
...
const runScenario = async (scenario, port) => {
  ...
  let app = await openApp({ file, profile: `${scenario.name}-edit`, port, indented: true })
  try {
    ...
    const current = await shape(app)
    assert.equal(current.topCount, 3)
    assert.equal(current.nestedCount, 0)
    assert.deepEqual(current.topTexts, ['alpha', 'beta', 'gamma'])
    ...
    const publication = state.owner.filter((entry) =>
      entry.phase === 'published' && entry.ok === true && entry.family === 'list-nested-single-child-bullet-outdent'
    )
    assert.equal(publication.length, 1, `${scenario.name} owner: ${JSON.stringify(state.owner)}`)
    assert.equal(publication[0].boundary,
      scenario.forced
        ? 'transaction-list-nested-single-child-bullet-outdent-forced-flush'
        : 'transaction-list-nested-single-child-bullet-outdent-markdown-updated')
    const preservation = state.preserve.find((entry) =>
      entry.reason === 'list-nested-single-child-bullet-outdented' &&
      entry.integrityProof?.kind === 'transaction-list-nested-single-child-bullet-outdent-proof'
    )
    ...
    assert.equal(preservation.integrityProof.movedSourceRow?.indent, '  ')
    ...
    assert.equal(preservation.integrityProof.step?.sliceSize, 1)
    ...
    assert.equal(state.coordinator.some((entry) =>
      entry.phase === 'published' && entry.family === 'list-nested-single-child-bullet-outdent'
    ), true)
  ...
  app = await openApp({ file, profile: `${scenario.name}-reopen`, port: port + 1, indented: false })
  try {
    assert.equal(await toggleSource(app), true)
...
try {
  ...
  completed = true
  console.log('PASS nested single child bullet outdent transaction UI: callback/forced physical Shift+Tab removes only two authored indent spaces while preserving marker/BOM/CRLF through source, save, disk and cold reopen')
} finally {
