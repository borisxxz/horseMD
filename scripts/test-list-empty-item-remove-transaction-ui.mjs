...
const root = `/tmp/horsemd-list-empty-item-remove-${process.pid}`
const basePort = Number(process.env.CDP_PORT || 15420 + (process.pid % 30))
...
const openApp = async ({ file, profile, port, expectedItems }) => {
  ...
  try {
    await waitFor(() => app.evaluate(`(() => {
      ...
      const texts = [...(list?.children || [])].map((item) =>
...
const focusEmptyItem = async (app) => {
...
const runScenario = async (scenario, port) => {
  ...
  let app = await openApp({
    ...
    port,
    expectedItems: ['left', '', 'right']
  })
  try {
    await clearDiagnostics(app)
    await focusEmptyItem(app)
    ...
    assert.equal(state.broad.some((entry) => entry.phase === 'publish' && entry.ok === false), false,
...
try {
  ...
  completed = true
  console.log('PASS list empty-item remove transaction UI: existing BOM/CRLF empty bullet removal publishes exactly once via callback or forced flush, saves exact authored bytes and cold reopens without the item or a br placeholder')
} finally {
