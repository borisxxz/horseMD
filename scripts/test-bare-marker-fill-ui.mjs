...
const root = `/tmp/horsemd-bare-marker-${process.pid}`
...
const imeType = async (send, text) => {
  const replacementId = `bare-marker-${compositionId++}`
...
// The divergent fixture mirrors the user's 无序列表测试.md: a bare `1` paragraph
// followed by an authored `-   1. 二哥...` row whose nested-list shape Crepe
// serializes differently (`* <br />` + `  1. 二哥...`).
...
try {
  ...
  // `1. ` + space to run the ordered-list input rule, then IME text. The
  // 0.13.69 regression wrote a BARE `1.` (invisible to the list machinery) and
  // the next text fill landed as a new `- 1. 色粉...` bullet, failing the
  // list-slot fingerprint.
  ...
  await imeType(app.send, '色粉嫩绿色负能量')
  await sleep(1100)

  const before = await diagnostics(app)
  console.log('BARE_MARKER_AFTER_INPUT:', JSON.stringify({
  ...
  const source = await waitFor(() => visibleSource(app), 'source textarea did not open').catch(async (error) => {
    console.error('BARE_MARKER_DIAGNOSTICS:', JSON.stringify(await diagnostics(app), null, 2))
    throw error
  })
  const after = await diagnostics(app)
  console.log('BARE_MARKER_SOURCE:', JSON.stringify({ source, toasts: after.toasts }))
  assert.equal(
    after.toasts.some((text) => /保存已暂停|无法安全映射|原文件未被覆盖|Save paused/.test(text)),
    false,
    'filling the ordered item after the input rule showed a sync warning'
  )
  assert.match(source, /啊额绿化\n\n1\. 色粉嫩绿色负能量\n\n1\n/m, 'the IME text must fill the 1. row with its marker space')
  assert.doesNotMatch(source, /- 1\. 色粉嫩绿色负能量/, 'the IME text must never become a new bullet row')
  assert.match(source, /-   1\. 二哥你来拿如果/, 'the divergent authored row must stay untouched')
  console.log('PASS bare-marker ordered item: input rule keeps the marker space and IME text fills in-row')
} finally {
