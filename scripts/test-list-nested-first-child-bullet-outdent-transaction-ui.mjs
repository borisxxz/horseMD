...
const root = `/tmp/horsemd-list-nested-first-child-outdent-${process.pid}`
const basePort = Number(process.env.CDP_PORT || 23420 + (process.pid % 30))
...
const scenarios = [
  {
    ...
    afterTop: ['alpha', 'beta', 'gamma', 'omega'],
    afterNested: ['delta'],
    nestedCount: 2,
    successorCount: 1,
    fixture: '\uFEFFbefore\r\n\r\n+ alpha\r\n+ beta\r\n  + gamma\r\n  + delta\r\n+ omega\r\n\r\nafter\r\n',
...
const openApp = async ({ file, profile, port, expectedTop, expectedNested }) => {
  ...
  try {
    await waitFor(async () => {
      ...
      return sameArray(current.topTexts, expectedTop) && sameArray(current.nestedTexts, expectedNested) &&
...
const runScenario = async (scenario, port) => {
  ...
  let app = await openApp({ file, profile: `${scenario.name}-edit`, port, expectedTop: scenario.beforeTop, expectedNested: scenario.beforeNested })
  try {
    ...
    const publication = state.owner.filter((entry) =>
      entry.phase === 'published' && entry.ok === true && entry.family === 'list-nested-first-child-bullet-outdent'
    )
    assert.equal(publication.length, 1, `${scenario.name} owner: ${JSON.stringify(state.owner)}`)
    assert.equal(publication[0].boundary,
      scenario.forced
        ? 'transaction-list-nested-first-child-bullet-outdent-forced-flush'
        : 'transaction-list-nested-first-child-bullet-outdent-markdown-updated')
    const preservation = state.preserve.find((entry) =>
      entry.reason === 'list-nested-first-child-bullet-outdented' &&
      entry.integrityProof?.kind === 'transaction-list-nested-first-child-bullet-outdent-proof'
    )
    ...
    assert.equal(preservation.integrityProof.successorCount, scenario.successorCount)
    assert.equal(preservation.integrityProof.targetIndex, 0)
    ...
    assert.equal(preservation.integrityProof.firstStep?.sliceSize, 3)
    assert.equal(preservation.integrityProof.secondStep?.sliceSize, 1)
    ...
    assert.equal(state.coordinator.some((entry) =>
      entry.phase === 'published' && entry.family === 'list-nested-first-child-bullet-outdent'
    ), true)
  ...
  app = await openApp({ file, profile: `${scenario.name}-reopen`, port: port + 1, expectedTop: scenario.afterTop, expectedNested: scenario.afterNested })
  try {
    assert.equal(await toggleSource(app), true)
...
try {
  ...
  completed = true
  console.log('PASS nested first-child bullet outdent transaction UI: 2-child callback and 3-child forced physical Shift+Tab preserve the moved successor rows as target-nested children while only removing the first target two-space indent, with authored marker/BOM/CRLF/source/save/disk/cold reopen intact')
} finally {
